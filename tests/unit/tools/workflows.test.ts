...
// `verifyRail` resolves and shells out to a REAL ngspice binary through
// src/simulation/runner.ts (detectNgspice -> execFile('ngspice', ['-v'])).
// Whether ngspice is on PATH is a property of the developer's machine, not of
// this code, so asserting either verdict against the live host makes the test
// pass or fail by accident. Auto-spy the runner module (original implementation
// preserved) so the two verifyRail tests below can pin the binary-resolution
// seam and assert BOTH the graceful-degradation and the success verdict
// deterministically. Every other test in this file leaves the real
// implementation in place.
vi.mock('../../../src/simulation/runner.js', { spy: true });
...
describe('Workflow Tools', () => {
  ...
  afterEach(() => {
    // Hand the auto-spied runner module back its real implementation so a pinned
    // ngspice verdict cannot leak into any other test.
    vi.mocked(detectNgspice).mockRestore();
    vi.mocked(runNgspiceDeck).mockRestore();
  });
  ...
  describe('easyeda_workflow_power_rail', () => {
    ...
    it('attaches a verification verdict when verifyRail is provided (ngspice unavailable)', async () => {
      // Pin the detection seam to "absent" so the graceful-degradation path is
      // exercised regardless of whether the host actually has ngspice installed.
      vi.mocked(detectNgspice).mockResolvedValue({
        available: false,
        error: 'spawn ngspice ENOENT',
      });
      ...
      // Degradation must be graceful: the workflow plan itself still succeeds.
      expect(result.blocked).toBe(false);
      // No deck may be executed once detection reported the binary missing.
      expect(vi.mocked(runNgspiceDeck)).not.toHaveBeenCalled();
    });

    it('attaches a verification verdict when verifyRail is provided (ngspice available)', async () => {
      // Same seam pinned to "present", with a canned operating-point stdout in
      // ngspice's real `print` format (see tests/unit/simulation/parser.test.ts).
      vi.mocked(detectNgspice).mockResolvedValue({ available: true, version: '44.2' });
      vi.mocked(runNgspiceDeck).mockResolvedValue({
        stdout: ['v(vin) = 5.000000e+00', 'v(vout) = 3.250000e+00'].join('\n'),
        stderr: '',
      });
      ...
      expect(result.verification).toBeDefined();
      expect(result.verification.available).toBe(true);
      expect(result.verification.error).toBeUndefined();
      expect(result.verification.ngspice_version).toBe('44.2');
      // 3.25V is ~1.5% below the 3.3V nominal, inside the 5% band.
      expect(result.verification.observed_voltage).toBeCloseTo(3.25, 6);
      expect(result.verification.within_tolerance).toBe(true);
      expect(result.verification.caveat).toMatch(/Simplified linear regulator model/);
      expect(vi.mocked(runNgspiceDeck)).toHaveBeenCalledTimes(1);
    });
  });
